...
impl App {
            ...
            PianoRollFocus::Navigation => {
                ...
                match key.code {
                    KeyCode::Esc | KeyCode::Char('q') => {
                        // Clear highlight if active, otherwise escape
                        if self.nav.clip_view.piano_roll.highlight_start.is_some() {
                            self.nav.clip_view.piano_roll.clear_highlight();
                        } else {
                            self.nav.escape();
                        }
                    }
                    ...
                    KeyCode::Char('d') => {
                        // Delete notes in highlighted columns
                        if let Some((start, end)) = self.nav.clip_view.piano_roll.highlight_range() {
                            self.delete_highlighted_notes(start, end);
                            self.nav.clip_view.piano_roll.clear_highlight();
                            self.send_clip_update();
                            dbg::user(&format!("piano roll: deleted notes in cols {}-{}", start + 1, end + 1));
                        }
                    }
                    KeyCode::Char('y') => {
                        // Yank (copy) notes in highlighted columns
                        if let Some((start, end)) = self.nav.clip_view.piano_roll.highlight_range() {
                            self.yank_highlighted_notes(start, end);
                            self.nav.clip_view.piano_roll.clear_highlight();
                            dbg::user(&format!("piano roll: yanked cols {}-{}", start + 1, end + 1));
                        }
                    }
                    KeyCode::Char('p') => {
                        // Paste yanked notes at highlighted position
                        if let Some((start, _end)) = self.nav.clip_view.piano_roll.highlight_range() {
                            self.paste_yanked_notes(start);
                            self.nav.clip_view.piano_roll.clear_highlight();
                            self.send_clip_update();
                            dbg::user(&format!("piano roll: pasted at col {}", start + 1));
                        } else {
                            // No highlight — paste at current column
                            let col = self.nav.clip_view.piano_roll.column;
                            self.paste_yanked_notes(col);
                            self.send_clip_update();
                            dbg::user(&format!("piano roll: pasted at col {}", col + 1));
                        }
                    }
                    KeyCode::Enter => {
    ...
    fn send_clip_update_for(&self, track_idx: usize, clip_idx: usize) {
    ...
    fn handle_delete_request(&mut self) {
        use crate::debug_log as dbg;
    ...
    fn execute_delete(&mut self, kind: ConfirmKind) {
        use crate::debug_log as dbg;
        match kind {
    ...
    fn yank_highlighted_notes(&mut self, col_start: usize, col_end: usize) {
        let col_count = self.nav.clip_view.piano_roll.column_count;
        if col_count == 0 { return; }
        let col_w = 1.0 / col_count as f64;
        let range_start = col_start as f64 * col_w;
    ...
    fn paste_yanked_notes(&mut self, paste_col: usize) {
        let col_count = self.nav.clip_view.piano_roll.column_count;
        if col_count == 0 { return; }
        let col_w = 1.0 / col_count as f64;
    ...
    fn do_save(&mut self, path_str: &str) {
    ...
    fn do_load(&mut self, path_str: &str) {
        let path = std::path::PathBuf::from(path_str);
