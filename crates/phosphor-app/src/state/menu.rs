...
pub const HELP_TOPICS: &[HelpTopic] = &[
    ...
    HelpTopic {
        ...
        body: &[
            ...
            Key("h / l", "bpm, record, loop, met, count, take"),
            Key("enter", "bpm: hold it \u{00b7} the switches: toggle"),
            Key("enter on take", "overdub \u{2194} re-record"),
            Note("Overdub layers each pass; re-record clears the loop"),
            Note("range first (one undo step brings it back)."),
            Key("esc", "release"),
            Gap,
            Heading("count-in"),
            ...
            Key("g", "marker grid: bar \u{00b7} beat \u{00b7} 1/8 \u{00b7} 1/16"),
            Key("j / k", "slide the whole brace \u{00b7} J/K leap by its length"),
            Key("enter", "loop on / off"),
            Key("esc", "done"),
            Gap,
            Heading("the brace as scissors"),
            Key("y", "lift what's between the markers \u{2014} every track"),
            Key("x", "lift and remove \u{00b7} one undo puts it all back"),
            Key("p", "stamp it at the brace \u{2014} clears what is"),
            Note("under the stamp, like a tape drop \u{2014} then"),
            Note("leapfrogs, so p p p lays copies back to back."),
            Key("P", "layer it instead: what is there stays"),
            Note("Chop a quarter-bar on the 1/16 grid and stamp"),
            Note("it four times: that is the whole gesture."),
            Gap,
            ...
            Note("peeled take back. The transport never stops. The top"),
            Note("bar counts the passes; R while playing punches in"),
            Note("where the music is, no rewind. Rec quant in the roll's"),
            Note("settings snaps takes to a grid as they land, and the"),
            Note("sustain pedal becomes real note length at commit."),
            Gap,
    ...
    HelpTopic {
        ...
        body: &[
            ...
            Gap,
            Heading("midi fx (chord and arp)"),
            Note("The add menu's \u{00b7}midi entries run on the notes,"),
            Note("before the instrument, live and on playback both."),
            Note("Recording still stores the keys you pressed."),
            Gap,
            Note("chord: below the split (C4), one key plays that"),
            Note("scale degree's whole chord \u{2014} 7ths, 9ths, lush \u{2014}"),
            Note("voiced to walk, not leap. Above it: melody \u{2014}"),
            Note("notes at or over the split pass through untouched."),
            Note("Knobs are live: a sounding chord re-voices as you"),
            Note("turn \u{2014} common tones hold, changed tones move."),
            ...
            Key("r", "in the editor: learn \u{2014} play a chord, lift,"),
            Note("and the row becomes exactly your voicing."),
            Note("The \u{201c}human\u{201d} knob on both devices loosens timing"),
            Note("and weight so generated notes sound like hands."),
    ...
    HelpTopic {
        ...
        body: &[
            ...
            Key("y / p", "off the grid: yank the whole pattern \u{00b7}"),
            Note("paste it into any slot \u{2014} on any track's"),
            Note("sequencer \u{2014} midi, length and all."),
